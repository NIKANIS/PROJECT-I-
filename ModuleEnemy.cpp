			...
			if (App->scene_chooseplayer->final_player2 == 1)
			{
				if (fliped == false)
					if (current_animation == &crowchpunch)
						enemy_punch_col = App->collision->AddCollider({ position.x + 40, position.y - 60, 35, 12 }, COLLIDER_ENEMY_ATTACK, App->enemy);
					else
					{
						enemy_punch_col = App->collision->AddCollider({ position.x + 50, position.y - 87, 45, 12 }, COLLIDER_ENEMY_ATTACK, App->enemy);
						enemy_col->SetPos(position.x + 17, position.y - 100);
					}

				else
					if (current_animation == &crowchpunch)
						enemy_punch_col = App->collision->AddCollider({ position.x - 24, position.y - 60, 35, 12 }, COLLIDER_ENEMY_ATTACK, App->enemy);
					else
					{
						enemy_punch_col = App->collision->AddCollider({ position.x - 30, position.y - 87, 45, 12 }, COLLIDER_ENEMY_ATTACK, App->enemy);
						enemy_col->SetPos(position.x + 15, position.y - 100);
					}
			}
			if (App->scene_chooseplayer->final_player2 == 2)
			{
	...
	if (kicking == true) {
		...
		if (at == 24)
		{
			if (App->scene_chooseplayer->final_player2 == 1)
			{
				if (fliped == false)
				{
					enemy_kick_col = App->collision->AddCollider({ position.x + 50, position.y - 85, 55, 17 }, COLLIDER_ENEMY_ATTACK, App->enemy);
					enemy_col->SetPos(position.x, position.y - 100);
				...
				{
					enemy_kick_col = App->collision->AddCollider({ position.x - 25, position.y - 85, 55, 17 }, COLLIDER_ENEMY_ATTACK, App->enemy);
					enemy_col->SetPos(position.x + 20, position.y - 100);
					enemy_col->rect.h = 110;
					enemy_col->rect.w = 50;
				}
			}
		...
		if (App->player->Health() != 0)
		{
			...
			else
			{
				...
				if (App->input->keyboard[SDL_SCANCODE_DOWN] == KEY_STATE::KEY_REPEAT && !jumping && !punching && !kicking && !specialattack_)
				{
					...
					if (fliped == true)
					{
						if (App->input->keyboard[SDL_SCANCODE_RIGHT] == KEY_STATE::KEY_REPEAT && !punching && !kicking && !specialattack_)
						{
				...
				if (App->input->keyboard[SDL_SCANCODE_DOWN] != KEY_STATE::KEY_REPEAT && current_animation != &crowchpunch) {
					lockX = false;
					crowchaction = false;
				}

				if (App->input->keyboard[SDL_SCANCODE_DOWN] == KEY_STATE::KEY_UP && current_animation != &crowchpunch)
				{
					current_animation = &idle;
				}
				...
				if (current_animation != &punchstanding && current_animation != &kickingstanding && current_animation != &crowchpunch)
				{
					if (App->scene_chooseplayer->final_player2 == 1)
					{
						if (current_animation == &crowch || current_animation == &crowchprotecc && crowchaction)
						{
							...
							{
								enemy_col->SetPos(position.x + 14, position.y - 67);
							}
						...
						else
						{
							if (!fliped)
							{
								enemy_col->SetPos(position.x + 5, position.y - 100);
								enemy_col->rect.h = 100;
								enemy_col->rect.w = 33;
							}
							else
							{
								enemy_col->SetPos(position.x + 27, position.y - 100);
								enemy_col->rect.h = 100;
								enemy_col->rect.w = 33;
							}
						}
...
void ModuleEnemy::Damage(const int damage, const int type)
{
